...
# Command handler to set the chat ID when the /start command is used
@bot.message_handler(commands=['start'])
def start_bot(message):
    global user_chat_id
    user_chat_id = message.chat.id
    print(f"Received /start command from chat ID: {user_chat_id}")  # Debugging output
...
def fetch_data():
    try:
        ...
        data = response.json()
        return data.get('data', {}).get('dexscreener_spents', [])  # Access 'new_pools' key in 'data'
...
def find_highest_volume_coin(coins):
    ...
    # Sort coins by 'buy_volume_5m' in descending order and return the first coin
...
def send_alert(coin_data):
    ...
    message = (
        ...
        f"🎯 **Exchange**: {base_token_info.get('exchange', 'Unknown')}\n"
        f"💡 **Market Cap**: ${base_token_info.get('market_cap', 'N/A')}\n"
        f"💧 **Liquidity**: ${base_token_info.get('liquidity', 'N/A')}\n"
        f"💰 **Token Price**: ${base_token_info.get('price', 'N/A')}\n"
        f"⛽️ **Pooled SOL**: {base_token_info.get('pooled_sol', 'N/A')} SOL\n"
        f"💰 **Total Supply**: {base_token_info.get('total_supply', 'N/A')}\n"
        f"💰 **Liquid Supply**: {base_token_info.get('liquid_supply', 'N/A')}\n"
    ...
    if user_chat_id:
        ...
        logging.info("No user has started the bot yet. Alert not sent.")
    ...
    while True:
        ...
        if coins:
            ...
            if highest_volume_coin:
                ...
                print(f"🚀 High Volume Coin: {base_token_info['name']} ({base_token_info['symbol']}) | Volume: {base_token_info['buy_volume_1m']} | Price: {base_token_info['price']} | Market Cap: {base_token_info['market_cap']} | Contract Address: {contract_address}")
